...
paymentRouter.post("/submit-payment" , async (req , res)=>{
    ...
    order.backUrl = `${process.env.HOSTNAME}/home` // must be a valid and active URL
    order.amount = amount // must be integer , and more or equal 75
    order.webhookUrl = `${process.env.HOSTNAME}/api/v1/payment/webhook` // this URL where receive the response 
